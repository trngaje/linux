...
#if defined(CONFIG_ARCH_MESON64_ODROID_COMMON)
	gpiod_set_pull(gpio_to_desc(charge->pdata->dc_det_pin),
			GPIOD_PULL_DIS);
#endif
...
static int rk817_charge_usb_init(struct rk817_charger *charge)
{
	...
	if (charge->pdata->extcon) {
		...
		ret = dwc_otg_charger_detect_register_notifier(&charge->bc_nb);
		if (ret) {
...
#if defined(CONFIG_ARCH_MESON64_ODROID_COMMON)
		dwc_otg_charger_detect_unregister_notifier(&charge->bc_nb);
#else
